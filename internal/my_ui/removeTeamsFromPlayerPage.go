...
func currentSelectionPageTfromP(playerContent *fyne.Container, teamContent *fyne.Container, sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {
...
func SelectionPageTfromP(sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {
	playersInTeam := 0
	...
	if playersInTeam > 0 {
		playerSelectionPageTfromPButton := widget.NewButton("Select a player", func() { w.SetContent(selectPlayerPageTfromP(sqlDB, db, w, a)) })
...
func selectPlayerPageTfromP(sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {

	returnToPlayerSelectionPageTfromPButton := widget.NewButton("Cancel", func() {
		w.SetContent(
			currentSelectionPageTfromP(
				SelectionPageTfromP(sqlDB, db, w, a), nil, sqlDB, db, w, a,
			),
		)
	})
	...
	for _, p := range sortedPlayers {
		...
		if len(player.TeamIDs) == 0 {
			...
			playerButton := widget.NewButton(player.Name, func() { w.SetContent(selectedPlayerPageTfromP(player, sqlDB, db, w, a)) })
...
func selectedPlayerPageTfromP(player *mt.Player, sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {
	...
	selectedPlayerButton := widget.NewButton(player.Name, func() {
		w.SetContent(selectPlayerPageTfromP(sqlDB, db, w, a))
	})
	...
	if len(player.TeamIDs) == 0 {
		...
		return selectPlayerPageTfromP(sqlDB, db, w, a)
	}

	// Now select a team
	selectTeamButton := widget.NewButton("Select a team", func() {
		w.SetContent(selectTeamPageTfromP(player, sqlDB, db, w, a))
	})
...
func selectTeamPageTfromP(player *mt.Player, sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {

	returnToTeamSelectionPageTfromPButton := widget.NewButton("Return to team selection", func() {
		w.SetContent(selectedPlayerPageTfromP(player, sqlDB, db, w, a))
	})
	...
	for _, t := range sortedTeams {
		...
		if _, ok := player.TeamIDs[team.ID]; ok {
			teamButton := widget.NewButton(team.Name, func() {
				...
				w.SetContent(selectedTeamPageTfromP(player, selectedTeams, sqlDB, db, w, a))
			})
...
func createTeamButtonsTfromP(player *mt.Player, team *mt.Team, sqlDB *msql.Database, db *mt.Database, selectedTeams map[int]*mt.Team, selectedTeamButtons []fyne.CanvasObject, w fyne.Window, a fyne.App) []fyne.CanvasObject {
...
func addAnotherTeamPageTfromP(player *mt.Player, alreadySelectedTeams map[int]*mt.Team, sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {

	returnToTeamSelectionPageTfromPButton := widget.NewButton("Cancel", func() {
		w.SetContent(selectedTeamPageTfromP(player, alreadySelectedTeams, sqlDB, db, w, a))
	})
...
func selectedTeamPageTfromP(player *mt.Player, selectedTeams map[int]*mt.Team, sqlDB *msql.Database, db *mt.Database, w fyne.Window, a fyne.App) *fyne.Container {
	returnToRemovePageButton := widget.NewButton("Return to the remove menu", func() {
		RemovePage(sqlDB, db, w, a)
	})
	...
	for _, t := range sortedSelectedTeams {
		team := t.Value
		selectedTeamButtons = createTeamButtonsTfromP(player, team, sqlDB, db, selectedTeams, selectedTeamButtons, w, a)
	}

	// Add another team in the team selection
	addAnotherTeamButton := widget.NewButton("Add another team", func() {
		w.SetContent(addAnotherTeamPageTfromP(player, selectedTeams, sqlDB, db, w, a))
	})
